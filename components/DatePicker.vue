<template>
  <div
    class="date-picker"
    :class="{ 'process-select': isProcessRangeSelect() }"
  >
    <slot name="header">
    ...
    <div class="days">
      <slot name="day-week" v-bind="{ daysWeek }">
        <ul class="days-week-container">
          <li class="day-week" v-for="day in daysWeek">
            {{ day }}
          </li>
        </ul>
      </slot>
      <slot name="days" v-bind="{ days, currentDay }">
        <transition name="fade" mode="out-in">
          <ul
            class="weeks"
            :key="currentDay.getFullYear() + '-' + currentDay.getMonth()"
          >
            <li
              class="week"
              v-for="(week, index) in days"
              :class="{
                'post-week-with-start-day': isWeekWithStartDay(days[index - 1]),
                'prev-week-with-end-day': isWeekWithEndDay(days[index + 1]),
              }"
            >
              <div
                v-for="(day, index) in week"
                @mousemove="changeIndicator(day)"
                class="indicator"
              ...
              >
                ...
                >
                  <button
                    ...
                    :class="{
                      today: isToday(day),
                      'other-month-day': isOtherMonthDay(day, currentDay),
...
function isHighlightDay(day: Date) {
  const index = highlightDays.value.findIndex(
