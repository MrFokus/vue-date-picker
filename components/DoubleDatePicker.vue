<template>
  <div class="double-dp">
    <slot name="left">
      <DatePicker class="left-dp" :mode="mode" v-model:current-day="currentDayLeft" v-model:highlight-days="unitedHighlightDays"
        v-model="date">
    ...
    <slot name="right">
      <DatePicker class="right-dp" :mode="mode" v-model:current-day="currentDayRight" v-model:highlight-days="unitedHighlightDays"
        v-model="date">
        <template #next-button>
...
const date = defineModel<ModelRange>({
  default: {
    start: null,
    end: null
  }
...
.double-dp {
  display: flex;
  box-shadow: 0px 0px 16px -2px rgba(14, 24, 41, 0.12), 0px 12px 16px -4px rgba(14, 24, 41, 0.12);
